...
function PodcastPlayer(id, playButtonId) {
  ...
  var loadFeed_ = function(url) {
    ...
    feed.load(function(result) {
      ...
      result.feed.entries.forEach(function(entry, i) {
        ...
        if (i == 0) {
          $('<li><a href="' + url).appendTo($('#episodes'));
        }
  ...
  var playAudio_ = function(url) {
    ...
    isSupp=myVid.canPlayType('video/mp4; codecs="avc1.4D401E, mp4a.40.2"');
    console.log(isSupp=="");
    if (isSupp==""){
    url = url.replace("mp4","ogg");
    }
    console.log(url); 
    audio_.src = url;
  ...
  var onLoadedData_ = function() {
    ...
    audio_.play();
    $('#play_button').addClass('playing').text('STOP THE RACKET');
  ...
  var togglePlayButtonUI_ = function() {
    ...
    if ($this.hasClass('playing')) {
      $this.removeClass('playing').text('RESUME STUPIDITY');
    } else {
      $this.addClass('playing').text('STOP THE RACKET');
    }
  };
